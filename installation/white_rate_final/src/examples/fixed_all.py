...
class state_table(object):
    ...
    def best_rate(self, current_mcs, current_width):
        ...
        if _candidates:
            return _candidates[0], self._max_width
        else:
    ...
    def pick_probe(self, current_mcs, current_width):
        ...
        for candidate_mcs in reversed(_valid_mcs):
            if (self.table[candidate_mcs]["lossless_tx_time"] < self.table[current_mcs]["average_tx_time"]) and (self.table[candidate_mcs]["succ_fails"] < _C) and (candidate_mcs != current_mcs):
