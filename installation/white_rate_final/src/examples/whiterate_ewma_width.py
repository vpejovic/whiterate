...
_C = 5#10 # successive fails tolerated
...
class state_table(object):
    ...
    def change_width(self, width):
        if width < self._max_width:
            ...
            for candidate_mcs in _valid_mcs:
                if (self.table[candidate_mcs]["average_tx_time"] < sys.maxint) and self.table[candidate_mcs]["average_tx_time"]>0:
                    self.table[candidate_mcs]["average_tx_time"] = (self.table[candidate_mcs]["average_tx_time"]*self.table[candidate_mcs]["width"])/width
                self.table[candidate_mcs]["width"] = self._max_width
                self.table[candidate_mcs]["lossless_tx_time"] = _coding_rate[candidate_mcs]*float(self._time_per_subcarrier)/(_bits_per_tone[candidate_mcs]*self._max_width)
        else:
            self._max_width = width
            for candidate_mcs in _valid_mcs:
                self.table[candidate_mcs]["lossless_tx_time"] = _coding_rate[candidate_mcs]*float(self._time_per_subcarrier)/(_bits_per_tone[candidate_mcs]*self._max_width)        
        print "Max width changed to "+str(width)+"\n"
