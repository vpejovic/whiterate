...
{
  return flex_ofdm_insert_preamble_sptr(new flex_ofdm_insert_preamble(fft_length,
								  preamble));
}

flex_ofdm_insert_preamble::flex_ofdm_insert_preamble
       (int fft_length,
	const std::vector<std::vector<gr_complex> > &preamble)
...
int
flex_ofdm_insert_preamble::general_work (int noutput_items,
...
#define write_out_flag() 			\
  ...
  while (no < noutput_items && ni < ninput_items){
    ...
    default:
      std::cerr << "flex_ofdm_insert_preamble: (can't happen) invalid state, resetting\n";
      enter_idle();
    }
  }
...
void
flex_ofdm_insert_preamble::enter_idle()
{
  d_state = ST_IDLE;
  d_nsymbols_output = 0;
  d_pending_flag = 0;
...
void
flex_ofdm_insert_preamble::enter_preamble()
{
  d_state = ST_PREAMBLE;
  d_nsymbols_output = 0;
  d_pending_flag = 1;
...
void
flex_ofdm_insert_preamble::enter_first_payload()
{
  d_state = ST_FIRST_PAYLOAD;
}

void
flex_ofdm_insert_preamble::enter_payload()
{
  d_state = ST_PAYLOAD;
}
...
void
flex_ofdm_insert_preamble::reset_preamble(std::vector<std::vector<gr_complex> > new_preamble)
{
